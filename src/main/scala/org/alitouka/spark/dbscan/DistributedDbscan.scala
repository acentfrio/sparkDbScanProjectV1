...
class DistributedDbscan (
  ...
  override protected def run(data: RawDataSet): DbscanModel = {
    ...
    val partiallyClusteredData = pointsWithNeighborCounts.mapPartitionsWithIndex (
      (partitionIndex, it) => {
        ...
        findClustersInOnePartition(it, partitionBoundingBox) //TODO тут уже без параметра
      },
      ...
      path => {
        partiallyClusteredData
          .map (
            x =>
              {
                //println(x._2.measure)
                x._2.coordinates.mkString(",") + "," + x._2.clusterId + "," + x._2.measure
              }
            )
          .saveAsTextFile(path)
      }
  ...
  private [dbscan] def findClustersInOnePartition (it: Iterator[(PointSortKey, Point)],
    ...
    points.map(pt=> //TODO стало
    {
      val immutablePoint = pt._2.toImmutablePoint
      immutablePoint.measure = pt._2.measure
      (new PointSortKey(pt._2), immutablePoint)
    }).iterator

    //points.map ( pt => (new PointSortKey(pt._2), pt._2.toImmutablePoint)).iterator TODO было
  }
  ...
  private def mergeClustersFromDifferentPartitions (partiallyClusteredData: RDD[(PointSortKey, Point)],
    ...
    DebugHelper.doAndSaveResult(partiallyClusteredData.sparkContext, "cpdb") {
      path => {
        pointsCloseToBoxBounds.map ( x =>  x.coordinates.mkString(",") + "," + x.clusterId + "," + x.measure ).saveAsTextFile(path)
      }
    }
